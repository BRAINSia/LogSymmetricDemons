...
void parseOpts(int argc, char * *argv, struct arguments & args)
{
  ...
  command.SetAcknowledgments(
    "This work stems from the author's CIFRE PhD thesis at INRIA (Asclepios team) and Mauna Kea Technologies");
  ...
  command.SetOption("NumberOfIterationsPerLevels", "i", false,
  ...
  command.SetOption(
    "MaximumUpdateStepLength", "l", false,
  ...
  command.SetOption(
    "UpdateRule", "a", false,
    "Type of update rule. 0: s <- s o exp(u) (diffeomorphic), 1: s <- s + u (additive, ITK basic), 2: s <- s o (Id+u) (compositive, Thirion's proposal?)");
  ...
  command.SetOption(
    "GradienType", "t", false,
    "Type of gradient used for computing the demons force. 0 is symmetrized, 1 is fixed image, 2 is warped moving image, 3 is mapped moving image");
  ...
  command.SetOption("UseHistogramMatching", "e", false,
                    "Use histogram matching prior to registration (e.g. for different MR scanners)");
    ...
    if( args.verbosity > 0 )
      ...
      if( !args.trueFieldFile.empty() )
        ...
        if( args.numIterations.size() > 1 )
          {
          std::cout << "You cannot compare the results with a true field in a multiresolution setting yet."
                    << std::endl;
          exit( EXIT_FAILURE );
          }
